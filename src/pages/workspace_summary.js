    ...
    <div className={`min-h-screen bg-[#FFFEF9] ${inter.className}`}>
      ...
      >
        <div className="relative w-screen inline-flex gap-1">
          ...
          />
          <button
            className="absolute right-10 translate-y-2 items-center"
      ...
      <div className="flex justify-start ml-8 me-8 mt-6">
        <div className="w-full h-[115px] px-[21px] py-5 bg-white rounded-[20px] border border-zinc-300 flex-col justify-start items-start inline-flex">
          <div className="text-stone-900 text-base font-normal font-['Rubik']">
            Balance
          </div>
          <div className="w-full justify-start items-center inline-flex">
            <div className="w-full text-stone-900 text-[32px] font-normal font-['Rubik']">
              THB
            </div>
            <div className="text-stone-900 text-[38px] font-normal font-['Rubik']">
              {balance}
            </div>
          </div>
        </div>
      </div>
      <div className="justify-start ml-8 mt-6 text-stone-900 text-[20px] font-medium font-['Rubik']">
        Summary
      </div>
      <div className="mt-4 mb-14">
        {userSearch.length === 0 &&
          info.map((summary, index) => (
            <Summary
              key={index}
              sum={parseInt(summary.sum)}
              username={summary.username}
              memberId={summary.member}
            />
          ))}
        ...
        {userSearch.length > 0 &&
          info.map(
            (summary, index) =>
              searchWord(summary.username, userSearch) === true && (
                <Summary
                  key={index}
                  sum={parseInt(summary.sum)}
                  username={summary.username}
