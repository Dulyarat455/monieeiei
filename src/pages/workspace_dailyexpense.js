  ...
  return (
    <div className={`min-h-screen bg-[#FFFEF9] ${inter.className}`}>
      ...
      <div
        ...
        className="flex mt-4 gap-2 justify-center items-center"
      >
        <div className="relative w-screen inline-flex gap-1">
          <input
            style={{ fontSize: "13px" }}
            className=" w-full h-[32px] p-2.5 bg-[#FFFEF9] rounded-lg border border-[#D9D9D9] focus:outline-none focus:border-purple-400 focus:ring-1 focus:ring-purple-400 placeholder-[#A6A6A6] placeholder:font-rubik pl-2 placeholder-text-xs text-neutral-400 placeholder-font-rubik placeholder-font-normal"
          ...
          <button
            className="absolute right-11 translate-y-2 items-center"
      ...
      <div
        ...
        className="sm:w-full md:w-full lg:w-full xl:w-full 2xl:w-full flex mt-2 justify-start items-center"
      >
        <div className="w-full sm:w-full md:w-full lg:w-full xl:w-full 2xl:w-full inline-flex justify-start items-center">
          ...
          <input
            style={{ fontSize: "13px" }}
            className="justify-end w-full sm:w-full md:w-full lg:w-full xl:w-full 2xl:w-full h-[32px] p-2.5 bg-[#FFFEF9] rounded-lg border border-[#D9D9D9] focus:outline-none focus:border-purple-400 focus:ring-1 focus:ring-purple-400 placeholder-[#A6A6A6] pl-2 placeholder-text-xs text-neutral-400 placeholder-font-rubik placeholder-font-normal"
            placeholder="MM/DD/YYYY"
            type="date"
            id="dateOut"
            name="dateOut"
      ...
      <div className="mt-4 mb-24">
        {tranSearch.length === 0 &&
          info.map((tran, index) => (
            <Transaction
              key={index}
              tran_name={tran.tran_name}
              tran_type={tran.tran_type}
              username_member={tran.username_member}
              count_member={tran.count_member}
              pocket_name={tran.pocket_name}
              category_name={tran.category_name}
              amount={tran.amount}
              bought_date={tran.bought_date}
              photo={tran.photo}
              owner_name={tran.owner_name}
              owner_id={tran.owner_id}
              transworkspace_id={tran.transworkspace_id}
            />
          ))}
        ...
        {tranSearch.length > 0 &&
          info.map(
            (tran, index) =>
              searchWord(tran.tran_name, tranSearch) === true && (
                <Transaction
                  key={index}
                  tran_name={tran.tran_name}
                  tran_type={tran.tran_type}
