      ...
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <div className="min-h-screen w-screen  bg-[#FAE392] rounded-t-[36px] mt-10 ">
          <div className=" inline-flex">
            <div className="font-rubik mt-10 ml-16 text-[500] text-[32px]">
              Last Notification
            </div>
            <button
              onClick={() => {
            ...
            >
              <Image alt="Icon" src={Icon} className=" ml-5 mt-10"></Image>
            </button>
          </div>
            ...
            {info.map(
              (noti, index) =>
                noti.date === dateNow && (
                  <Listnoti1
                    key={index}
                    time={noti.time_stamp}
                    date={formatDate(noti.date)}
                    description={noti.description}
                    status={noti.notification_type}
                  ></Listnoti1>
                )
            )}
          </div>
