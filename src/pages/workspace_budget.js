...
export default function Workspace_dailyexpense(){
    ...
    return(
        <div className={`min-h-screen bg-[#FFFEF9] ${inter.className}`}>
            ...
            <Slidebar/>
            {/* Search */}
            <div style={{width:'screen', justifyContent:'space-between', padding:'0 30px'}} className="flex mt-4 gap-1 justify-center items-center">
                <div className="relative w-screen inline-flex gap-1">
                    ...
                    />
                    <button className="absolute right-10 translate-y-2 items-cente inline-flexr" onClick={handleIconClick}>
                        <svg 
               ...
               {ownerStatus === 1 && <Link href="/setproportion">
            ...
            <div style={{justifyContent:'space-between',padding:'0 30px'}} className="w-screen flex mt-2 justify-start items-center">
                <div className="w-full sm:w-full md:w-full lg:w-full xl:w-full 2xl:w-full inline-flex justify-start items-center">
                    ...
                    <input style={{fontSize:'13px',width:'100%'}} className="justify-end w-full sm:w-full md:w-full lg:w-full xl:w-full 2xl:w-full h-[32px] p-2.5 bg-[#FFFEF9] rounded-lg border border-[#D9D9D9] focus:outline-none focus:border-purple-400 focus:ring-1 focus:ring-purple-400 placeholder-[#A6A6A6] placeholder-text-xs text-neutral-400 placeholder-font-rubik placeholder-font-normal"
                    placeholder="MM/DD/YYYY"
                    type="date"
                    name="dateOut"
                    onChange={changeHandler}
            ...
            <Scroll />
            <div className="mb-24">
            {categorySearch.length === 0  && info.map((budget,index) => (     
            ...
            {categorySearch.length > 0  && info.map((budget,index) => (     
                ( searchWord(budget.category_name,categorySearch) === true   && <Category key={index} category_name = {budget.category_name} budget = {budget.budget} actual = {budget.actual} remaining = {budget.remaining} />)
            ))}
